...
DownloadsViewUI.DownloadElementShell.prototype = {
     ...
     */
    showButton(type) {
        let { commandName, label, panelL10nId, iconClass } =
            gDownloadElementButtons[type];

        this.buttonCommandName = commandName;

        this._downloadLinks.innerHTML = "";

        if (commandName != "downloadsCmd_showBlockedInfo") {
            this.createLink(type);
            this.createLink("removeFile");
        }

        this._downloadLinks.removeAttribute("hidden");
    },
    ...
    createLink(type) {
        ...
        link.setAttribute("value", label);

        link.addEventListener("click", (e) => {
            this.doCommand(commandName);
        });

        this._downloadLinks.appendChild(link);
    },

    hideButton() {
        this._downloadLinks.setAttribute("hidden", "true");
    },
